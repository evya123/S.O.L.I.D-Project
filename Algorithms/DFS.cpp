...
std::string DFS::search(ISearchable<State *> &searchable) {
    ...
    AlgoUtils<State *>::ShortestPath(root, goal);
    return AlgoUtils<State *>::printPath(root, goal);

}

void DFS::DepthFirstSearch(State *start, State *target,
                           ISearchable<State *> &searchable) {
    if (!target->isVisit()) {
        start->setVisit(true);
...
std::queue<State *>
DFS::fromVecToQueue(State *root, std::vector<State *> &toBeCleared) {
    std::queue<State *> ret;
